...
# By default, the first target (this) is the one that will be executed if no
# target is specified
compile: $(TARGET)
...
# <source_dir>/<file>.o depends on <source_dir>/<file>.c
# This rule has higher priority than the previous one because it is more specific
$(SRC_DIR)/%.o: $(SRC_DIR)/%.c
	$(CC) $(CFLAGS) -I $(INC_DIR) -c $< -o $@

# Rule to link object files and create the executable (target): depends on all
# object files
$(TARGET): $(OBJS)
